...
#
# i.e. compare two versions of the same panel to characterise how the panel has evolved
#
##################################################

#This script has been written to analyse different versions of the same panel and determine change 
#It achieves this by taking one version of a panel, and comparing it to the version of the panel from the month prior
...
def main(file_path: str):
	...
	temp_month_file = parameters_df.iloc[8,1]
	temp_panel_loc = parameters_df.iloc[9,1]
	return(temp_name, temp_version, temp_outdir, temp_sdate, temp_edate,temp_id_loc, temp_id_file, temp_month_loc, temp_month_file, temp_panel_loc )
...
if __name__ == "__main__":
	...
	monthly_file = the_returned[8]
	panel_loc = the_returned[9]
...
###################################################
	...
	reclassification_summary = "%s/%s(%s)-%s_GeneChange_Summary.txt" % (parent_location1, panel_name, panel_id, source)
...
###################################################
		...
		individual_panel_location = "%s/%s_%s_%s" % ( panel_loc, source, panel_id, panel_name)
		...
		for gene_line in file_specific_version:
			...
			if dge_status != "":
				if ensg_id in df_this_month.values:
					...
					#df_this_month = pd.concat(df_temp, ignore_index=True)	
